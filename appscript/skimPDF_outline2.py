...
def customOutline(structure, uniq_category, level_refs):
    ...
    if structure[0] != []:
        ...
        for i in range(0, len(b[0])):
            ...
            if type(b[2][i]) == list:
                if len(b[2][i]) > 2:
                    print b[2][i] + '\t' + level_refs[i] + '.\t' + b[0][i] + ':' + b[1][i]
            else:
                print b[2][i] + '\t\t\t' + level_refs[i] + '.\t' + b[0][i] + ':' + b[1][i]
...
def setFormats(structure, i= -1):
    ...
    while step == 'y':
        if step == 'y':
            x = multiQuestion('Set format to:', formats)
        ...
        for it in options:
            print '-' + it
        print '\n---------------------\n'
        step = askQuestion('Add/change any more formats?', '(y/n)', ['y', 'n'])
    if i == -1: structure[3].append(options)
    else: structure[3][i] = options
    return structure
...
def checkFormat(data, i, structure):
    ...
    if structure[0].count(data[5][i]) != 0 and structure[1].count(data[2][i]) != 0:
        # format=''
        # stop=False
        for j in range(0, len(structure[0])):
    ...
    data[3][i]=x
    data_type_color=str(data[0][i])+":"+str(data[2][i])
    if data_type_color == 'k.highlight_note:orange':
        if x.find('=') != -1:
            boldText=x[:x.find('=')]
            formatText(boldText,boldText,'bold')
    '''
    return data
...
def makeSkimOutline():
    ...
    listFout.listFout(uniq_category, 'uniq_category.txt')
    listFout.listFout(data, 'data.txt')
    ...
    try:
        ...
        offset = setOffset(len(docPages))
        structure = [], [], [], [], offset
        
    structure = setOutline(data, structure, uniq_category, openFile)
