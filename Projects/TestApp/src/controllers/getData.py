...
def getSiteUrls(url):
  ...
  for url in range(0, len(links)):
    try:
        ...
        e2 = z.find('>', s2)
        temp2 = z[s2:e2]
    sites.append(str(temp1))
    siteUrls.append(str(temp2))
    if temp1 == None:
...
def getLocationSubCate(location, top_category):
  ...
  x = str(siteHTML.find(text=top_category).parent.get('href')).replace('/', '')
  subLinks = siteHTML.find('div', id=x).findChildren('a')
...
def getSub_Category(Location_Url, top_category, sub_category):
    ...
    Category_Name = categories[Cate_index]  
